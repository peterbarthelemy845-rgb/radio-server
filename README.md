...
Add Station remains public: no registration or sign-in is required. Every accepted submission receives a random ST- station code and keeps the entered contact email. The normal waiting-for-approval popup is unchanged. Admins see the code and can assign the approved station to an owner account later. If already signed in with the same contact email, the submission is associated with that account; email alone never claims an existing account. Client-supplied owner IDs and codes do not assign ownership. Duplicate stream submissions are rejected instead of overwriting existing records. An account may own multiple stations, but an email cannot create multiple accounts.
